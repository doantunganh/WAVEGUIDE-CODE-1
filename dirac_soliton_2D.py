...
#    index_taken = math.floor(step_num/wgc.FRAME_NUM)
    ...
    nvec2 = np.arange(start = 0, stop = wgc.COL_NUM, step = 1)
    mvec2 = np.arange(start = 0, stop = wgc.ROW_NUM, step = 1)
    ...
    a_n[wgc.COL_NUM-1] = 1j ** (wgc.COL_NUM + 1) * coeff_1 / math.cosh((nvec2[wgc.COL_NUM-1] - ncenter) / wgc.BEAMWIDTH);
    ...
    a_m[wgc.ROW_NUM-1] = 1j ** (wgc.ROW_NUM + 1) * coeff_1 / math.cosh((mvec2[wgc.ROW_NUM-1] - ncenter) / wgc.BEAMWIDTH);
